...
const Navbar = () => {
    ...
    return (
        <nav className="md:px-4 flex items-center justify-between bg-background lg:bg-transparent rounded-md bg-clip-padding backdrop-filter lg:backdrop-blur-xl lg:bg-opacity-30 fixed w-full md:h-16 h-16">
            <div className={`borderb w-full md:h-16 h-16 md:p-0 px-2 flex items-center justify-between bg-transparent relative ${isOpen ? 'pointer-events-none' : ''}`}>
                ...
                <div className={`buttons flex gap-3 ${isOpen ? 'hidden' : ''}`}>
                    ...
                    <div className="flex items-center justify-center space-x-1">
                        ...
                        <Link to="https://github.com/reachmohdaltaf" target="_blank">
                            <Button className="text-xl px-0 flex items-center hover:text-foreground">
                                <img 
                                    src="/github.svg" 
                                    className={`h-12 ${theme === "light" ? "invert-75 hover:invert-90" : "invert-0 hover:invert-40"}`} 
                                    alt="GitHub" 
                                />
                            </Button>
                        </Link>
            ...
            {isOpen && (
                <div className="absolute top-16 z-50 items-center w-full bg-secondary h-screen p-4 rounded-md shadow-lg flex flex-col gap-4">
                    {navButtons.map((btn, index) => (
                        <Link to={btn.link} key={index} onClick={() => setIsOpen(false)}>
                            <Button
                                size={"xl"}
